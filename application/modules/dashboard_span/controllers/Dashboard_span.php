...
class Dashboard_pok extends CI_Controller{
    ...
    function index($id = NULL){
      if ($id == NULL) {
        $x['title'] = 'Parent';
        $data = $this->span_model->get_span_data()->result();
        // var_dump($data);
        // exit();
        ...
        $x['title'] = $id;
        if (is_numeric($id)) {
          $data = $this->span_model->get_out_data($id)->result();
        } else {
          $data = $this->span_model->get_unit_data($id)->result();
        }
      }
