...
class pok_model extends CI_Model{
  ...
  public function get_unit_data($biro) {
		...
		// $this->db->select('unit_pok.id_b, unit_pok.id, unit_pok.nama, sum(out_pok.pagu) as pagu, sum(out_pok.realisasi) as realisasi, sum(out_pok.kembali) as kembali, MAX(out_pok.tgl) as tgl');
		...
		$result = $this->db->query("SELECT unit_pok.id_b, unit_pok.id, unit_pok.nama, sum(out_pok.pagu) as pagu, sum(out_pok.realisasi) as realisasi, sum(out_pok.kembali) as kembali, MAX(out_pok.tgl) as tgl from out_pok JOIN unit_pok on out_pok.id_u = unit_pok.id where unit_pok.id_b = $id_b GROUP BY unit_pok.id");
